...
def critical_multi_i(eta_i, output):
    #g_i = nx.erdos_renyi_graph(int(1E2),0.3)
    g_i = nx.read_graphml('../data/g_power.graphml')
    #g_i = g_random = nx.read_graphml('../data/g_random.graphml')
    g_name_i = 'power_unifom2'
...
def main():
    ...
    with open('../results/'+'theta_result_power_uiform2.pickle', 'wb') as handle:
