...
def rho_theta(g_i, theta_i, no_sim):
    p = mp.Pool(30)
    x_list = np.arange(0.01,1,0.01)
    rho_list = [p.apply(cascade_theta,args=(g_i, theta_i,x_i)) for x_i in np.repeat(x_list,no_sim)]
    pickle.dump(rho_list, open('../results/real_data_3/cascade_power_{}.pickle' .format(theta_i), 'wb'))
 


def main():
    g_i = nx.read_graphml('../data/g_power.graphml') 
    for theta_i in [i/100 for i in list(range(50,75,5))]:
        rho_theta(g_i, theta_i, 500)
